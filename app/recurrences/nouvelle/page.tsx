  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const { error: itemsError } = await supabase
        ...
        .insert(lines.map(l => ({
          recurring_order_id: recurrence.id,
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Nouvelle récurrence</h1>
          <p className="text-gray-500 mt-1">Commande automatique pour un client régulier</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          >
            <Save size={20} />
            {loading ? 'Enregistrement...' : 'Enregistrer la récurrence'}
          </button>
        </div>
      </form>
    </div>
