...
import {
  Plus, FileText, CheckCircle, ChevronDown, ChevronUp, Trash2, X,
  Square, CheckSquare, RotateCcw, LayoutList, Table2, Link2, Pencil, Save,
} from 'lucide-react';
...
interface BDCLineRef {
  stock_item_id: string;
  prix_unitaire: number;           // prix de réception (BDC)
...
export default function FacturesPage() {
  ...
  // Édition des prix (pour factures liées à un BDC)
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editPrices, setEditPrices] = useState<Record<string, number>>({}); // lineId → prix

  // Formulaire nouvelle facture manuelle
  ...
  async function load() {
    const [{ data: inv }, { data: sup }, { data: itm }] = await Promise.all([
      supabase.from('supplier_invoices')
        .select('*, supplier:suppliers(nom), lines:supplier_invoice_lines(*, stock_item:stock_items(nom, unite)), order:purchase_orders(id, date, lines:purchase_order_lines(stock_item_id, prix_unitaire, quantite_recue, quantite_commandee))')
  ...
  function toggleSelectAll() {
    setSelectedIds(selectedIds.size === filteredInvoices.length ? new Set() : new Set(filteredInvoices.map(i => i.id)));
  }

  // ── Validation ────────────────────────────────────────────────────────────────

  async function _validerOne(inv: Invoice) {
    await supabase.from('supplier_invoices').update({ statut: 'validee' }).eq('id', inv.id);

    if (inv.order_id && inv.order) {
      // Facture issue d'une réception BDC → stock déjà mis à jour, on corrige seulement le PMP si les prix diffèrent
      const bdcLines = inv.order.lines || [];
      for (const line of inv.lines || []) {
        const bdcLine = bdcLines.find(bl => bl.stock_item_id === line.stock_item_id);
        const receptionPrice = bdcLine?.prix_unitaire ?? line.prix_unitaire;
        const invoicePrice = line.prix_unitaire;
        if (Math.abs(invoicePrice - receptionPrice) > 0.001) {
          const { data: item } = await supabase.from('stock_items')
            .select('stock_actuel, prix_moyen_pondere').eq('id', line.stock_item_id).single();
          if (item && (item.stock_actuel || 0) > 0) {
            const correctedPmp = (item.prix_moyen_pondere || 0) + line.quantite * (invoicePrice - receptionPrice) / item.stock_actuel;
            await supabase.from('stock_items').update({ prix_moyen_pondere: Math.max(0, correctedPmp) }).eq('id', line.stock_item_id);
          }
        }
      }
    } else {
      // Facture manuelle → comportement classique : entrée en stock
      for (const line of inv.lines || []) {
        ...
        const newPmp = newQty > 0 ? ((item.stock_actuel || 0) * (item.prix_moyen_pondere || 0) + line.quantite * line.prix_unitaire) / newQty : line.prix_unitaire;
        await supabase.from('stock_items').update({ stock_actuel: newQty, prix_moyen_pondere: newPmp }).eq('id', line.stock_item_id);
        await supabase.from('stock_movements').insert({
          stock_item_id: line.stock_item_id, type: 'entree_facture',
          quantite: line.quantite, prix_unitaire: line.prix_unitaire,
          reference_id: inv.id, reference_type: 'supplier_invoice', date: inv.date_facture,
        });
      }
    }
  }
  ...
  async function validerFacture(inv: Invoice) {
    if (!confirm(`Valider la facture ${inv.numero || inv.id.slice(0, 8)} ?${inv.order_id ? '\nLe stock a déjà été mis à jour à la réception. Seul le PMP sera corrigé si les prix diffèrent.' : '\nLe stock sera mis à jour.'}`)) return;
    await _validerOne(inv);
    load();
  }

  // ── Édition des prix ──────────────────────────────────────────────────────────

  function startEditPrices(inv: Invoice) {
    const prices: Record<string, number> = {};
    for (const l of inv.lines || []) { if (l.id) prices[l.id] = l.prix_unitaire; }
    setEditPrices(prices);
    setEditingId(inv.id);
  }

  async function saveEditPrices(inv: Invoice) {
    setSaving(true);
    for (const [lineId, prix] of Object.entries(editPrices)) {
      await supabase.from('supplier_invoice_lines').update({ prix_unitaire: prix }).eq('id', lineId);
    }
    const newTotal = (inv.lines || []).reduce((s, l) => s + l.quantite * (l.id ? (editPrices[l.id] ?? l.prix_unitaire) : l.prix_unitaire), 0);
    await supabase.from('supplier_invoices').update({ total: newTotal, numero: fNumeroEdit || inv.numero }).eq('id', inv.id);
    setEditingId(null); setEditPrices({}); setFNumeroEdit('');
    setSaving(false); load();
  }

  const [fNumeroEdit, setFNumeroEdit] = useState('');

  function startEdit(inv: Invoice) {
    startEditPrices(inv);
    setFNumeroEdit(inv.numero || '');
  ...
  async function bulkSetStatus(statut: 'brouillon' | 'validee') {
    if (statut === 'validee') {
      ...
      const toReset = invoices.filter(i => selectedIds.has(i.id) && i.statut === 'validee');
      if (toReset.length > 0 && !confirm(`Remettre ${toReset.length} facture(s) validée(s) en brouillon (le stock ne sera pas recalculé) ?`)) return;
    }
    setBulkLoading(true);
    const ids = Array.from(selectedIds);
    if (statut === 'validee') {
      const toValidate = invoices.filter(i => ids.includes(i.id) && i.statut === 'brouillon');
      for (const inv of toValidate) await _validerOne(inv);
    } else {
  ...
  function addLine() { setFLines(p => [...p, { stock_item_id: '', quantite: 1, prix_unitaire: 0 }]); }
  function updateLine(idx: number, k: string, v: unknown) { setFLines(p => p.map((l, i) => i === idx ? { ...l, [k]: v } : l)); }
  ...
  async function saveInvoice() {
    ...
    const { data: inv, error } = await supabase.from('supplier_invoices').insert({
      supplier_id: fSupplierId, numero: fNumero || null, date_facture: fDate, statut: 'brouillon', total: fTotal,
    }).select().single();
    if (!error && inv) {
      await supabase.from('supplier_invoice_lines').insert(fLines.map(l => ({ ...l, invoice_id: inv.id })));
      setShowForm(false); setFSupplierId(''); setFNumero(''); setFLines([]);
      load();
    }
    setSaving(false);
  }
  ...
  const filteredInvoices = filterMonth ? invoices.filter(i => i.date_facture?.slice(0, 7) === filterMonth) : invoices;
  const availableMonths = Array.from(new Set(invoices.map(i => i.date_facture?.slice(0, 7)).filter(Boolean))).sort((a, b) => b!.localeCompare(a!)) as string[];
  const allSelected = filteredInvoices.length > 0 && selectedIds.size === filteredInvoices.length;
  const someSelected = selectedIds.size > 0;

  // ── Rendu lignes de facture ────────────────────────────────────────────────────

  function renderLines(inv: Invoice) {
    const isEditing = editingId === inv.id;
    const fromBDC = !!inv.order_id;
      ...
      <div className="border-t border-gray-50 px-4 pb-3">
        ...
        {fromBDC && inv.order && (
          <div className="flex items-center gap-1.5 mt-3 mb-2">
            ...
            <span className="text-xs text-blue-600 font-medium">
              BDC lié — réception du {new Date(inv.order.date).toLocaleDateString('fr-FR')}
            </span>
            <span className="text-xs text-gray-400 ml-1">· quantités bloquées à la réception</span>
          </div>
        )}
        ...
        {isEditing && (
          <div className="mt-2 mb-3">
            <input
              value={fNumeroEdit}
              onChange={e => setFNumeroEdit(e.target.value)}
              placeholder="N° de facture fournisseur"
              className="w-full px-3 py-2 border border-blue-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
            />
          </div>
        )}
          ...
          <table className="w-full text-sm mt-2 min-w-[320px]">
            <thead>
              <tr className="text-xs text-gray-400">
                ...
                <th className="text-right pb-1">Qté reçue</th>
                <th className="text-right pb-1">
                  {isEditing ? <span className="text-blue-600 font-semibold">Prix facture ✏</span> : 'Prix unit.'}
                </th>
                <th className="text-right pb-1">Total</th>
                {fromBDC && !isEditing && <th className="text-right pb-1 text-gray-300">Prix BDC</th>}
              </tr>
            </thead>
            <tbody>
              {(inv.lines || []).map((l, i) => {
                const bdcLine = inv.order?.lines?.find(bl => bl.stock_item_id === l.stock_item_id);
                const receptionPrice = bdcLine?.prix_unitaire;
                const currentPrice = l.id ? (editPrices[l.id] ?? l.prix_unitaire) : l.prix_unitaire;
                const priceChanged = receptionPrice !== undefined && Math.abs(currentPrice - receptionPrice) > 0.001;

                return (
                  <tr key={i} className="border-t border-gray-50">
                    <td className="py-1.5 text-gray-700">
                      {l.stock_item?.nom} <span className="text-gray-400">({l.stock_item?.unite})</span>
                    </td>
                    {/* Quantité bloquée si BDC lié */}
                    <td className="text-right text-gray-700 tabular-nums">
                      {l.quantite}
                      {fromBDC && <span className="ml-1 text-gray-300 text-xs">🔒</span>}
                    </td>
                    {/* Prix : éditable en mode édition */}
                    <td className="text-right tabular-nums">
                      {isEditing && l.id ? (
                        <input
                          type="number" min={0} step={0.01}
                          value={editPrices[l.id] ?? l.prix_unitaire}
                          onChange={e => setEditPrices(p => ({ ...p, [l.id!]: parseFloat(e.target.value) || 0 }))}
                          className="w-24 px-2 py-1 border border-blue-300 rounded-lg text-sm text-right focus:outline-none focus:ring-1 focus:ring-blue-400 bg-blue-50"
                        />
                      ) : (
                        <span className={priceChanged ? 'text-orange-600 font-semibold' : 'text-gray-700'}>
                          {currentPrice.toFixed(2)}
                          {priceChanged && <span className="ml-1 text-orange-400 text-xs">↑</span>}
                        </span>
                      )}
                    </td>
                    <td className="text-right font-semibold text-gray-900 tabular-nums">
                      {(l.quantite * currentPrice).toFixed(2)}
                    </td>
                    {fromBDC && !isEditing && (
                      <td className="text-right text-gray-300 tabular-nums text-xs">
                        {receptionPrice?.toFixed(2) ?? '—'}
                      </td>
                    )}
                  </tr>
                );
        ...
        {inv.statut === 'brouillon' && (
          <div className="flex gap-2 mt-3">
            {isEditing ? (
              <>
                <button onClick={() => { setEditingId(null); setEditPrices({}); setFNumeroEdit(''); }}
                  className="flex-1 py-2 border border-gray-200 rounded-xl text-sm text-gray-500 hover:bg-gray-50">
                  Annuler
                </button>
                <button onClick={() => saveEditPrices(inv)} disabled={saving}
                  className="flex-1 py-2 bg-blue-600 text-white rounded-xl text-sm font-semibold hover:bg-blue-700 disabled:opacity-40 flex items-center justify-center gap-1.5">
                  <Save size={13} /> Enregistrer les prix
                </button>
              </>
            ) : (
              <>
                <button onClick={() => startEdit(inv)}
                  className="flex-1 py-2 border border-blue-200 text-blue-700 rounded-xl text-sm font-semibold hover:bg-blue-50 flex items-center justify-center gap-1.5">
                  <Pencil size={13} /> Saisir N° & prix facture
                </button>
                <button onClick={() => validerFacture(inv)}
                  className="flex-1 py-2 bg-green-600 text-white rounded-xl text-sm font-semibold hover:bg-green-700 flex items-center justify-center gap-1.5">
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-gray-400">
            ...
            {filterMonth ? ` · ${new Date(filterMonth + '-01').toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' })}` : ''}
          </p>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-blue-200 p-4 space-y-4">
          <div className="flex items-center justify-between">
            <p className="font-semibold text-gray-900">Nouvelle facture manuelle</p>
      ...
      {someSelected && (
        <div className="sticky top-0 z-10 bg-gray-900 text-white rounded-2xl px-4 py-3 flex items-center gap-3 shadow-lg">
          ...
          <button onClick={() => setSelectedIds(new Set())} className="p-1.5 text-gray-400 hover:text-white"><X size={16} /></button>
        </div>
      )}
            ...
            <table className="w-full text-sm">
                  ...
                  return (
                    <React.Fragment key={inv.id}>
                      <tr className={`border-t border-gray-50 hover:bg-gray-50 transition-colors ${selected ? 'bg-blue-50/60' : ''}`}>
                        ...
                        <td className="px-3 py-2.5">
                          <div className="flex items-center justify-end gap-1">
                            {!valide && <button onClick={() => supprimerFacture(inv.id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg"><Trash2 size={13} /></button>}
                      ...
                      {open && (inv.lines || []).length > 0 && (
                        <tr className="bg-gray-50"><td colSpan={8} className="px-4 pt-0 pb-3">{renderLines(inv)}</td></tr>
                      )}
                    </React.Fragment>
                  );
                })}
              ...
              <tfoot className="border-t border-gray-200 bg-gray-50">
                <tr className="text-xs text-gray-500 font-semibold">
                  ...
                  <td className="px-3 py-2.5 text-right">{filteredInvoices.reduce((s, i) => s + i.total, 0).toFixed(2)} MAD</td>
                  <td colSpan={2} />
                </tr>
              </tfoot>
            </table>
                ...
                <div key={inv.id} className={`bg-white rounded-2xl border overflow-hidden transition-all ${selected ? 'border-blue-400 ring-1 ring-blue-300' : valide ? 'border-gray-100' : 'border-blue-200'}`}>
                  <div className="flex items-start gap-3 px-4 py-3">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      {!valide && <button onClick={() => supprimerFacture(inv.id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg"><Trash2 size={14} /></button>}
                      {open ? <ChevronUp size={16} className="text-gray-400" /> : <ChevronDown size={16} className="text-gray-400" />}
                    </div>
                  </div>
                  {open && (inv.lines || []).length > 0 && renderLines(inv)}
                </div>
              );
            })}
          </div>
