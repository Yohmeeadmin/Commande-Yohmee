...
interface ProdCategory { id: string; nom: string; ordre: number; }

function ProdCategoriesTab() {
  const [cats, setCats] = useState<ProdCategory[]>([]);
  const [loading, setLoading] = useState(true);
  const [newName, setNewName] = useState('');
  const [adding, setAdding] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  ...
  async function load() {
    setLoading(true);
    const { data } = await supabase.from('production_categories').select('*').order('ordre');
    setCats(data || []);
    setLoading(false);
  }
  ...
  async function add() {
    ...
    setAdding(true);
    const maxOrdre = cats.reduce((m, c) => Math.max(m, c.ordre), 0);
    const { data, error } = await supabase.from('production_categories').insert({ nom: newName.trim(), ordre: maxOrdre + 1 }).select().single();
    if (!error && data) { setCats(p => [...p, data]); setNewName(''); }
    setAdding(false);
  }
  ...
  async function saveEdit(id: string) {
    if (!editName.trim()) return;
    await supabase.from('production_categories').update({ nom: editName.trim() }).eq('id', id);
    setCats(p => p.map(c => c.id === id ? { ...c, nom: editName.trim() } : c));
    setEditingId(null);
  }

  async function remove(id: string) {
    if (!confirm('Supprimer cette catégorie de production ?')) return;
    await supabase.from('production_categories').delete().eq('id', id);
    setCats(p => p.filter(c => c.id !== id));
  }

  async function move(cat: ProdCategory, dir: 'up' | 'down') {
    const sorted = [...cats].sort((a, b) => a.ordre - b.ordre);
    const idx = sorted.findIndex(c => c.id === cat.id);
    const swapIdx = dir === 'up' ? idx - 1 : idx + 1;
    if (swapIdx < 0 || swapIdx >= sorted.length) return;
    const other = sorted[swapIdx];
    await Promise.all([
      supabase.from('production_categories').update({ ordre: other.ordre }).eq('id', cat.id),
      supabase.from('production_categories').update({ ordre: cat.ordre }).eq('id', other.id),
    ]);
    setCats(p => p.map(c => {
      if (c.id === cat.id) return { ...c, ordre: other.ordre };
      if (c.id === other.id) return { ...c, ordre: cat.ordre };
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-gray-500">Ces catégories s'appliquent aux mises en place et sous-recettes (ex : Crème, Pâte feuilletée, Appareil…).</p>
      ...
      {sorted.length === 0 && <p className="text-sm text-gray-400 text-center py-4">Aucune catégorie de production. Ajoutez-en ci-dessous.</p>}

      <div className="flex gap-2 pt-2">
        <input type="text" value={newName} onChange={e => setNewName(e.target.value)}
          onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); add(); } }}
          placeholder="Ex : Crème, Pâte levée, Appareil, Ganache…"
          className="flex-1 px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
        <button onClick={add} disabled={adding || !newName.trim()}
          className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-semibold hover:bg-blue-700 disabled:opacity-50">
          <Plus size={16} /> Ajouter
...
export default function CategoriesPage() {
  ...
  const [tab, setTab] = useState<'ateliers' | 'categories' | 'mp' | 'prod'>('ateliers');
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        ...
        <div className="flex border-b border-gray-100">
          {(['ateliers', 'categories', 'prod', 'mp'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              ...
              {t === 'ateliers' ? 'Ateliers' : t === 'categories' ? 'Catégories PF' : t === 'prod' ? 'Catégories Prod' : 'Catégories MP'}
            </button>
          ))}
        </div>
