...
export default function ChargesVariablesPage() {
  ...
  const [charges, setCharges] = useState<ChargeVar[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [form, setForm] = useState({ categorie: 'Divers', libelle: '', montant: '', date_charge: '' });
  const [saving, setSaving] = useState(false);

  const load = useCallback(async () => {
    setLoading(true);
    const { data } = await supabase.from('charges_variables').select('*')
      .eq('mois', mois).order('date_charge', { ascending: false }).order('created_at', { ascending: false });
    setCharges((data as ChargeVar[]) || []);
    setLoading(false);
  }, [mois]);
  ...
  async function save() {
    ...
    setSaving(true);
    await supabase.from('charges_variables').insert({
      mois,
      categorie: form.categorie,
      libelle: form.libelle.trim(),
      montant: Number(form.montant),
      date_charge: form.date_charge || null,
    });
    setSaving(false);
    setModalOpen(false);
    setForm({ categorie: 'Divers', libelle: '', montant: '', date_charge: '' });
    load();
  }

  async function remove(id: string) {
    if (!confirm('Supprimer cette charge ?')) return;
    await supabase.from('charges_variables').delete().eq('id', id);
    load();
  }
  ...
  const byCategorie = new Map<string, ChargeVar[]>();
  charges.forEach(c => {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-black text-gray-900 leading-none">Charges variables</h1>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
          <p className="text-xs text-gray-400 mb-1.5">Nombre d&apos;entrées</p>
          <p className="text-2xl font-black text-gray-900">{charges.length}</p>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
          <p className="text-xs text-gray-400 mb-1.5">Total du mois</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
          <p className="text-sm text-gray-300 mb-3">Aucune charge ce mois-ci</p>
          <button onClick={() => setModalOpen(true)} className="text-sm text-blue-600 font-semibold">+ Ajouter</button>
        </div>
      ) : (
        <div className="space-y-3">
                    ...
                    <div className="min-w-0">
                      ...
                      {c.date_charge && (
                        <p className="text-xs text-gray-400 mt-0.5">
                          {new Date(c.date_charge).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}
                        </p>
                      )}
                    </div>
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-2xl px-5 py-4 flex items-center justify-between">
            <p className="text-sm font-bold text-blue-800">Total charges variables</p>
        ...
        <>
          ...
          <div className="fixed inset-x-4 top-1/2 -translate-y-1/2 z-50 bg-white rounded-2xl shadow-xl p-5 max-w-sm mx-auto">
            <div className="flex items-center justify-between mb-5">
              <p className="font-semibold text-gray-900">Nouvelle charge</p>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input value={form.libelle} onChange={e => setForm(f => ({ ...f, libelle: e.target.value }))}
                  className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-blue-400"
                  placeholder="Ex : Réparation four" autoFocus />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 mb-1.5 block">Montant (MAD) *</label>
                <input type="number" value={form.montant} onChange={e => setForm(f => ({ ...f, montant: e.target.value }))}
                  className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-blue-400"
                  placeholder="1500" min={0} />
              </div>
            </div>
            <button onClick={save} disabled={saving || !form.libelle.trim() || !form.montant}
