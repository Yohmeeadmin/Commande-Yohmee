...
export default function DriverViewPage() {
  ...
  async function confirmPartialWithBackorder(createBackorder: boolean) {
    ...
    setOrders(prev => prev.map(o =>
      o.id === deliveryOrder.id
        ? { ...o, status: 'livree', items: o.items.map(i => ({ ...i, quantity_delivered: deliveryQtys[i.id] ?? i.quantity_ordered })) }
        : o
    ));
    if (createBackorder) {
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              ...
              <button onClick={closeDeliveryModal} className="p-2 rounded-xl hover:bg-gray-100 text-gray-400">
                <X size={20} />
              </button>
            </div>
